...
マウスのホイール部分で曲をクリックするとお気に入り登録できます。お気に入りを一つ以上登録した状態で「お気に入りのみ」を選択するとお気に入りの曲のみを再生します。
(最初の曲は現在再生中の曲の次にお気に入りの曲なので再生中の曲より下にお気に入りの曲がないとプレイリストが終了します)
曲をマウスの右クリックでクリックすると次に再生する曲を指定できます。「お気に入りのみ」モードの時はお気に入り以外の曲を指定できないため、通常モードに直してから指定してください。
