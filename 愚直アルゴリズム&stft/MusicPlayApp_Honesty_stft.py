...
class AudioFile:
        ...
        def play(self):
            ...
            data_pre = self.wf.readframes(self.chunk)
            data=''
            ...
            while len(data_pre)!=0 and info.quit==False:
                ...
                if msvcrt.kbhit():
                    ...
                    if self.stream.is_active():
                        if data != '':
                            data_pre=data
                        if info.Key>0 and info.Key<=6 and info.Quickness==1:#Keyが0から6の通常再生は短時間フーリエ変換の方が音質が良いのでこちらを採用
                            data = self.wf.readframes(int(self.chunk))
                            if len(data)==(self.chunk)*4:
                                data_pre = gen_stft(data_pre,info.Quickness,info.r12,self.wf.getframerate(),self.wf.getnframes())
                        else: #それ以外はクロスフェードを利用した愚直アルゴリズム
                            if info.Quickness/info.r12 < 1:
                                if self.wf.tell()==info.last:
                                    info.quit=True
                                else:
                                    if self.wf.tell()-int(self.chunk*(1-(info.Quickness/info.r12)))-4096 >=info.head:
                                        self.wf.setpos(self.wf.tell()-int(self.chunk*(1-(info.Quickness/info.r12)))-4096)
                                    data = self.wf.readframes(int(self.chunk)+4096)
                            elif info.Quickness/info.r12 > 1:
                                if self.wf.tell()==info.last:
                                    info.quit=True
                                else:
                                    if self.wf.tell()-4096 >= info.head:
                                        self.wf.setpos(self.wf.tell()-4096)
                                    dummy = self.wf.readframes(int(self.chunk*((info.Quickness/info.r12)-1)))
                                    data = self.wf.readframes(int(self.chunk)+4096)
                            else:
                                data = self.wf.readframes(int(self.chunk)+4096)
                            if info.Quickness/info.r12 != 1 and len(data)==(self.chunk+4096)*4:
                                data_pre,data = gen_xfade_honesty(data_pre,data,8192,int(self.wf.getframerate()*info.r12))
...
class WinodwClass(tk.Frame):
    def __init__(self,master):
        ...
        tk.Button(master, text="pitch+0.1", fg = "red",command=partial(backgroundprocess,b'up'),font=("",20)).grid(row=1, column=0, padx=10, pady=10)
        ...
        info.label.pack()
        getplaytime(master)
        tk.Button(master, text="プログラム終了", fg = "deep pink",command=lambda: master.destroy() if info.thread_play is None else print("PlayListを終了させてください"),font=("",20)).grid(row=6, column=3, padx=10, pady=10)
...
if __name__ == "__main__":
    ...
    info.Quickness=round(info.Quickness,1)
    info.basedirname=os.path.dirname(os.path.abspath("__file__"))
    print(info.basedirname)
    help()
    r = 2**(1/12)
    info.r12=r**np.float(info.Key)
    start_windowthread()
